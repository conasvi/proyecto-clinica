...
router.post('/agregar', (req, res) => {
    ...
    connection.query(`INSERT INTO medicos (cedula, nombres, apellido, especialidad,consultorio, correo) VALUES (${cedula},'${nombre}', '${apellido}', '${especialidad}', '${consultorio}', '${correo}')`, (error, result) => {
        if (error) {
...
router.post('/actualizar/:cedula', (req, res) => {
    ...
    const consultorio = req.body.consultorio;
    const especialidad = req.body.especialidad;
    connection.query(`UPDATE medicos SET nombres='${nombre}',apellido='${apellido}', correo='${correo}',consultorio = '${consultorio}',especialidad='${especialidad}' WHERE cedula=${cedula}`, (error, results) => {
        if (error) {
            console.log("Error en la consulta", error)
            res.status(500).send("Error en la consulta ")
        } else {
...
router.get('/eliminar/:cedula', function (req, res, next) {
    const cedula = req.params.cedula
    connection.query(`DELETE FROM cita_medicas WHERE id_medico=${cedula}`, (error, results) => {
        if (error) {
            console.log("Error en la consulta", error)
            res.status(500).send("Error en la consulta")
        } else {
